...
internal static class AddHasBeenNotifiedMigration
{
    internal static async Task EnsureColumns(ISession session, ILogger logger)
    {
        ...
        foreach (var column in new[] { "has_been_notified", "is_estimate" })
        {
            ...
            try
            {
                await session.ExecuteAsync(new SimpleStatement($"ALTER TABLE {keyspace}.order_book ADD {column} boolean"));
